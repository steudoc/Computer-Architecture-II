...
Reset_Handler   PROC
				...
				MOV R10, #0		;somma delle differenze
				MOV R11, #0		;contatore carte con guadagno
				MOV R5, #0		;indice per loop sulle carte
				
loop_cards
				CMP R5, R4
				BGE end_program
				
				LDR R6, [R0, R5, LSL #2] 	;ID carta corrente
				
				MOV R7, R2
				BL find_price	;purchase price stored in R8
				MOV R9, R8
				
				MOV R7, R3
				BL find_price	;current price stored in R8
				MOV R12, R8
...
search_price_loop
				...
				MOV R8, #0		; non trovato, ritorna 0
				B end_price			
found_price
				LDR R8, [R0]
end_price
